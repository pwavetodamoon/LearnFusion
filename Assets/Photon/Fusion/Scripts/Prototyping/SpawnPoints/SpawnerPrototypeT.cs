...
public class SpawnerPrototype<T> : SimulationBehaviour, IPlayerJoined, IPlayerLeft, ISpawned, ISceneLoadDone where T : Component, ISpawnPointPrototype {
  ...
  protected Dictionary<PlayerRef, List<Fusion.NetworkObject>> _spawnedLookup = new Dictionary<PlayerRef, List<Fusion.NetworkObject>>();
  ...
  public void Spawned() {
    ...
    if (Object) {
      if (_AllowClientObjects && StateAuthority != AuthorityOptions.Server) {
                Fusion.NetworkObject playerNetworkObject = TrySpawn(Runner, Runner.LocalPlayer);
  ...
  void PlayerJoined(NetworkRunner runner, PlayerRef player) {
    ...
    if (_AllowClientObjects && StateAuthority != AuthorityOptions.Server) {
      return;
        ...
        Fusion.NetworkObject playerNetworkObject = TrySpawn(runner, player);
  ...
  public Fusion.NetworkObject TrySpawn(NetworkRunner runner, PlayerRef player) {
  ...
  protected virtual void RegisterPlayerAndObject(PlayerRef player, Fusion.NetworkObject playerObject) {
    if (!_spawnedLookup.TryGetValue(player, out var objList)) {
      objList = new List<Fusion.NetworkObject>();
